...
RoboFDM::RoboFDM() : newLoaded_(true), first_init_(false) {
	load_candidate_dirs(candDirs_);
}
...
double RoboFDM::get_inrev_risky_area()
{
	...
	for (auto f : faces(poly))
	{
		...
		if (isInrev[f->patch_id()])  sumInrevArea += f->area();
	}
...
py::array_t<double> RoboFDM::render() {
	...
	for (auto i = 0; i < evalResults.size(); ++i) 
	{
		...
		for (auto j = 0; j < evalResults[i].size(); ++j)
		{
			...
			arrEvalRes[dim * curIdx + 2] = std::max(0.0, std::min(tmpEval.get<3>() / std::sqrt(bsphere.squared_radius()), 1.0));
			arrEvalRes[dim * curIdx + 3] = std::max(0.0, std::min( tmpEval.get<4>() / std::sqrt(bsphere.squared_radius()), 1.0));
