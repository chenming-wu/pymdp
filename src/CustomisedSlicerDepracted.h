...
//
//
// Edited by Chenming Wu for research purpose.
// Date: 2017/02/16

#ifndef CUSTOMISED_MESH_SLICER_H
#define CUSTOMISED_MESH_SLICER_H
...
namespace CGAL {
    ...
    class Polygon_mesh_slicer
    {
        ...
        struct Polyline_visitor {
            ...
            Polyline_visitor(TriangleMesh& tmesh,
                    ...
                    , out(out) {
                itr_alv = std::back_inserter(al_vertices);
            }

            std::vector< Point_3 > current_poly;
            std::list<std::vector< AL_vertex_descriptor> > al_vertices;
            std::back_insert_iterator<std::list<std::vector< AL_vertex_descriptor> > >  itr_alv = std::back_inserter(al_vertices);
            std::vector< AL_vertex_descriptor> current_al_vertices;

            void start_new_polyline()
            {
                current_poly.clear();
                current_al_vertices.clear();
            }
            void add_node(AL_vertex_descriptor node_id)
            {
                current_al_vertices.push_back(node_id);
                ...
                AL_graph& al_graph) const
        {
            ...
            if (face(hd, m_tmesh) != graph_traits::null_face())
            {
                ...
                // ?????new_insertion???????null_vertex()?????????
                // ???????new_insertion???????halfedge?????
                // compare operator ??face?????????????
                if (!new_insertion)
                {
    ...
    public:
        ...
        */
        template <class OutputIterator, class FacetIterator>
        OutputIterator operator() (const Plane_3& plane,
                                   OutputIterator out, FacetIterator faceOut) const
        {
            ...
            // array iedges usually is non-empty, but array vertices is empty all the time.
            //std::cout << "iedges = " << iedges.size() << " ,vertices = " << vertices.size() << std::endl;
            ...
            if (!UseParallelPlaneOptimization || app_info.first == -1)
            {
                ...
                //std::cout << "al_vertices = " << visitor.al_vertices.size() << std::endl;
                for (auto lit = visitor.al_vertices.begin(); lit != visitor.al_vertices.end(); ++lit) {
                    int cntId = 1;
                    std::vector<face_descriptor> vecFaces;
                    vecFaces.reserve(num_faces(m_tmesh) / 10);

                    for (auto vit = lit->begin(); vit != lit->end(); ++vit) {
                        AL_vertex_info v = al_graph[*vit];
                        const vertex_descriptor* vd_ptr = boost::get<vertex_descriptor>(&v);
                        if (vd_ptr == nullptr) {
                            edge_descriptor ed = boost::get<edge_descriptor>(v);
                            //face(halfedge(ed, m_tmesh), m_tmesh)->vecIDs.push_back(cntId);
                            //face(opposite(halfedge(ed, m_tmesh), m_tmesh), m_tmesh)->vecIDs.push_back(cntId);
                            face_descriptor f = face(halfedge(ed, m_tmesh), m_tmesh);
                            f->id() = cntId;
                            vecFaces.push_back(f);
                            f = face(opposite(halfedge(ed, m_tmesh), m_tmesh), m_tmesh);
                            f->id() = cntId;
                            vecFaces.push_back(f);
                        }
                    }
