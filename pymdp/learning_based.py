...
class BGS:
    ...
    def feedforward_search(self):
        ...
        for sel in cur_sel:
            ...
            cur_plane = all_r[cur_idx, 6::]
            ret_poly = run_cut_process(self.b_polys[poly_idx], cur_plane, self.export)
            if ret_poly == None:
                continue
    ...
    def start_search(self):
        ...
        self.b_trajs.prepare_data(os.path.join(self.output_folder, os.path.basename(self.filename)[:-4]))

        if self.export:
            self.b_trajs.export_best_segmentation(os.path.join(self.output_folder, os.path.basename(self.filename)[:-4]), self.export_polys)


if __name__ == "__main__":
    proc = BGS(filename='kitten.off')
    proc.set_beam_width(10)
    proc.set_output_folder("output")
    proc.start_search()